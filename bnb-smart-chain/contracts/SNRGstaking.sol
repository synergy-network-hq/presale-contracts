...
contract SNRGStaking is Ownable2Step, ReentrancyGuard, Pausable {
    ...
    function stake(uint256 amount, uint64 duration) external nonReentrant whenNotPaused {
        ...
        uint256 balanceAfter = SNRG.balanceOf(address(this));

        uint256 actualReceived = balanceAfter - balanceBefore;
        if (actualReceived == 0) revert InsufficientBalance();

        uint256 reward = (actualReceived * rewardBps) / 10000;

        if (rewardReserve < promisedRewards + reward) {
            emit ReserveShortfall(promisedRewards + reward, rewardReserve);
        ...
        userStakes[msg.sender].push(
            Stake({amount: actualReceived, reward: reward, endTime: endTime, withdrawn: false})
        );

        promisedRewards += reward;

        emit Staked(msg.sender, stakeIndex, actualReceived, reward, endTime);
    }
