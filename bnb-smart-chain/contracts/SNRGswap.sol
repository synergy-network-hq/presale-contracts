...
contract SNRGSwap is Ownable2Step, ReentrancyGuard, Pausable {
    ...
    /// @notice FIX M001: Cryptographic anchor - hash commitment to on-chain state
    bytes32 public burnCommitment;
    /// @notice FIX M003: Timestamp of last finalization to enforce cooldown on reopens
    uint256 public lastFinalizedAt;
    /// @notice FIX M003: Minimum delay before allowing finalization reopen (7 days)
    uint256 public constant REOPEN_COOLDOWN = 7 days;
     ...
     * @dev FIX M001: Cryptographically anchors merkle root to on-chain burned accounting
     *      FIX SCWE-057: adds timelock and ties to on-chain burned accounting
     */
    function finalize() external onlyOwner {
        ...
        // FIX M001: Create cryptographic commitment to on-chain state
        // This binds the merkle root to the actual burned amounts recorded on-chain
        burnCommitment = keccak256(abi.encodePacked(
            proposedRoot,
            totalBurned,
            block.timestamp,
            block.chainid
        ...
        lastFinalizedAt = block.timestamp; // FIX M003: Track finalization time
        proposedRoot = bytes32(0);
        proposedAt = 0;
     ...
     * @dev FIX M003: Enforces cooldown period to prevent instant clearing and DOS
     *      Clears the finalized root and enforces a new delay window for the replacement root
    ...
    function reopenFinalization(bytes32 newRoot) external onlyOwner {
        ...
        // FIX M003: Enforce minimum cooldown before allowing reopen
        // This prevents instant clearing of finalized roots and gives users time to claim
        if (block.timestamp < lastFinalizedAt + REOPEN_COOLDOWN) {
            revert ReopenCooldownActive();
        }

        bytes32 previousRoot = merkleRoot;
        finalized = false;
        merkleRoot = bytes32(0);
        burnCommitment = bytes32(0); // Clear commitment when reopening
