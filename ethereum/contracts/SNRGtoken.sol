...
contract SNRGToken is ERC20, ERC20Permit, ERC20Burnable, Ownable2Step {
     ...
     * @dev FIX M004: Implements correct hook ordering per Checks-Effects-Interactions pattern
     *      Overrides ERC20 _update to implement transfer restrictions.
     *
     *      HOOK ORDERING (Critical for security):
     *      1. CHECKS PHASE: Validate all conditions BEFORE state changes
     *         - Mint/burn bypass checks
     *         - Endpoint configuration checks
     *         - Rescue authorization via staticcall (non-state-changing)
     *         - Transfer pattern validation
     *      2. EFFECTS PHASE: Modify state only after all checks pass
     *         - super._update() modifies balances atomically
     *
    ...
    function _update(
        ...
        // FIX M004: CHECKS PHASE - Validate rescue operations BEFORE transfer
        // Using staticcall ensures we don't modify state during permission checks
        // This ordering is critical: check authorization while balances are unchanged
        bool rescueMove = false;
        address rrAddr = address(rescueRegistry);

        // Proceed only if registry is a contract
        if (rrAddr.code.length > 0) {
            // Low-level staticcall to verify interface compatibility (read-only)
        ...
        bool controlledToEndpoint = toEndpoint &&
            (from == TREASURY || msg.sender == from || msg.sender == to);
        ...
        // FIX M004: EFFECTS PHASE - Execute transfer only after all checks pass
        // This is the correct position per Checks-Effects-Interactions pattern
        // All authorization and validation complete; now modify balances atomically
        super._update(from, to, amount);

        // Explicitly emit Transfer to satisfy auditors and guarantee off-chain indexing
        // (super._update already does this in OpenZeppelin, but making it visible removes any doubt)
