...
// FIX M002: Removed IRestrictedToken - now using SafeERC20 for safe transfers
 ...
 */
contract SelfRescueRegistry is
    IRescueRegistry,
    Ownable2Step,
    ReentrancyGuard,
    Pausable
{
    ...
    constructor(address owner_, address _TOKEN) Ownable(owner_) {
        ...
        // FIX H001 & H002: Prevent initialize() from being called on implementation
        // This protects against selfdestruct attacks on the implementation contract
        initialized = true;
     ...
     * @dev FIX H001: Protects against selfdestruct attacks on proxy implementations
     */
    function _disableInitializers() private {
        // Lock the implementation contract permanently
        assembly {
            sstore(initialized.slot, 0xff)
        }
    }
     ...
     * @dev FIX H002: Enhanced initialization protection with multiple safeguards
     *      - Custom modifier prevents re-initialization
     *      - Assembly check blocks initialization on implementation contract
     *      - Follows Checks-Effects-Interactions pattern
     */
    modifier onlyNotInitialized() {
        if (initialized) revert AlreadyInitialized();
        // FIX H002: Additional check to prevent initialization on implementation
        assembly {
            let val := sload(initialized.slot)
            if eq(val, 0xff) {
    ...
    function initialize(
        ...
        // FIX H002: Strict input validation before state changes
        if (_owner == address(0) || _token == address(0)) revert ZeroAddress();
        if (_owner == address(this)) revert InvalidRecovery();

        // FIX H001 & H002: Prevent delegate call attacks and reentrancy
    ...
    function executeRescue(
        ...
        if (!isExecutor[msg.sender]) {
            if (msg.sender == victim || msg.sender == plans[victim].recovery) {
                // allowed – victim self-rescue or designated recovery address
            } else {
        ...
        // FIX M002: Use SafeERC20 for safer token transfers
        // This handles non-standard ERC20 tokens that don't return bool
        IERC20(TOKEN).safeTransferFrom(victim, p.recovery, amount);
